...
class BaseFeatureGroup(metaclass=ABCMeta):
    ...
    def add(self, features: SparkDataFrame) -> SparkDataFrame:
        ...
        """
        assert self.aggregation_level in features.columns, "Error: aggregation column not available in intermediate feature dataframe"

        feature_group = self._compute_feature_group(self._load_source_data(), aggregation_level=self.aggregation_level)
        return features.join(feature_group.alias(self.alias + "-" + self.aggregation_level), on=self.aggregation_level, how="left")
